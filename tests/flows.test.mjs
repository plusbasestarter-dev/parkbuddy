...
test('car can be selected without GPS, saved, and deleted without resurrection',async t=>{
  const app=create(t);await settle(()=>app.document.querySelector('[data-detail]'));app.click('.nav [data-go=car]');assert.equal(app.document.getElementById('carMapWrap').hidden,false);assert.equal(app.document.getElementById('findCar').hidden,true);
  ...
  app.click('#car [data-action=deleteCar]');await settle(()=>core.readCar(app.disk)?.deleted&&!core.readCar(app.disk)?.pending);assert.equal(app.document.getElementById('carMapWrap').hidden,false);app.click('.nav [data-go=home]');app.click('.nav [data-go=car]');await settle();assert.equal(core.readCar(app.disk).deleted,true);
});
